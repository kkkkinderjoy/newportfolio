    ...
    <div className="w-full h-[6rem] bg-white text-slate-800 p-3">        
      <div className="max-w-full mx-auto lg:mx-10 flex-wrap flex justify-between">
          ...
          <ul className='flex dark:bg-transparent'>
                    ...
                    <li className="flex items-center ml-1 dark:bg-transparent"><NavLink to="/projects" className='nav text-sm lg:text-xl relative'> Projects </NavLink></li>
                    <button type='radio' className='text-slate-400 shadow-off p-4 rounded-[70px] w-14 h-10 font-sm  flex items-center justify-center ml-3 dark:shadow-on' onClick={()=>{dispatch(toggleTheme())}}>
                    <input type="hidden"/>{darkMode === "light" ? "off" : "on"}
                    </button>
          </ul>
     </div>
    </div>
